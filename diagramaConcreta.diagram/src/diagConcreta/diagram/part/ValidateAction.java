...
public class ValidateAction extends Action {
	...
	*/
	public ValidateAction(IWorkbenchPage page) {
		setText(diagConcreta.diagram.part.Messages.ValidateActionMessage);
		this.page = page;
	}
	...
	*/
	public static void runValidation(View view) {
		try {
			if (diagConcreta.diagram.part.DiagConcretaDiagramEditorUtil.openDiagram(view.eResource())) {
			...
			diagConcreta.diagram.part.DiagConcretaDiagramEditorPlugin.getInstance().logError("Validation action failed", //$NON-NLS-1$
					e);
		}
	}
	...
	*/
	public static void runValidation(DiagramEditPart diagramEditPart, View view) {
		...
		diagConcreta.diagram.providers.DiagConcretaValidationProvider.runWithConstraints(txDomain, new Runnable() {
	...
	*/
	private static void validate(DiagramEditPart diagramEditPart, View view) {
		...
		if (target != null) {
			diagConcreta.diagram.providers.DiagConcretaMarkerNavigationProvider.deleteMarkers(target);
		}
	...
	*/
	private static void createMarkers(IFile target, IStatus validationStatus, DiagramEditPart diagramEditPart) {
		...
		diagConcreta.diagram.part.DiagConcretaDiagramEditorUtil.LazyElement2ViewMap element2ViewMap = new diagConcreta.diagram.part.DiagConcretaDiagramEditorUtil.LazyElement2ViewMap(
				diagramEditPart.getDiagramView(),
				collectTargetElements(rootStatus, new HashSet<EObject>(), allStatuses));
		for (Iterator it = allStatuses.iterator(); it.hasNext();) {
			IConstraintStatus nextStatus = (IConstraintStatus) it.next();
			View view = diagConcreta.diagram.part.DiagConcretaDiagramEditorUtil.findView(diagramEditPart,
	...
	private static void createMarkers(IFile target, Diagnostic emfValidationStatus, DiagramEditPart diagramEditPart) {
		...
		for (Iterator it = emfValidationStatus.getChildren().iterator(); it.hasNext();) {
			...
			if (data != null && !data.isEmpty() && data.get(0) instanceof EObject) {
				...
				View view = diagConcreta.diagram.part.DiagConcretaDiagramEditorUtil.findView(diagramEditPart, element,
						element2ViewMap);
				addMarker(diagramEditPart.getViewer(), target, view.eResource().getURIFragment(view),
	...
	*/
	private static void addMarker(EditPartViewer viewer, IFile target, String elementId, String location,
		...
		diagConcreta.diagram.providers.DiagConcretaMarkerNavigationProvider.addMarker(target, elementId, location,
